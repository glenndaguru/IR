...
class attacker():
    ...
    def get_service(self):#This fetches the service that will be sent an attack
        service_count=self.data.count_services()#count number of services in db
...
for i in range(0,server_count,1):
    ...
    action=data.attack_details['actions']
    # print(service)
    # print(servers)
    # print(action)
    ...
    # items={'Service':service['service_name'],'IP':servers['']['ip-addr'],'User':servers['']['username'],'Password':servers['']['password'],'Method':action['']['method'],}
    # my_dic[i]=''
